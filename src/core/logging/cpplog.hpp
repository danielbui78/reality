...
#ifdef _WIN32
// Removed because it conflicts with Windows
...
namespace cpplog
{
    ...
    struct LogData
    {
        ...
        LogData(loglevel_t logLevel) : streamBuffer(), stream(&streamBuffer), level(logLevel)
#ifdef CPPLOG_SYSTEM_IDS
...
// Only exit in debug mode if CPPLOG_FATAL_EXIT_DEBUG is set.
    ...
    public:
        ...
        virtual bool sendLogMessage(LogData* logData)
        {
            ...
            // Flush every line logged when we debug
            #ifndef NDEBUG
